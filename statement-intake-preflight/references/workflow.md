...
- `coverage_hints.statement_period_years`, source-referenced `period_intervals`, `unresolved_periods`, and `date_evidence` support the requested year. Schema 1.3 classifies each date as statement-period, movement, opening-boundary, generated-on, or certificate-issued; only statement periods establish coverage. Compact headers such as `1 SEP - 30 SEP` or `1 SEP - 30 SEP (30 días)` need an in-range source anchor before they resolve; the parenthetical count is decorative and never supplies a year or interval. Each interval retains independent start/end references; resolve an unresolved period only through its stable opaque ID.
...
- `account_hints` describe one account when scope is `one-account`, or the user confirmed one-account scope without supplying an account number. An exact stacked `Número de cuenta` label may bind only the nearest identifier directly beneath it in the same visual column; amount, phone, page, and reference-shaped values remain rejected.
...
| `possible-missing-statement-period` | review | Two or more source-referenced period labels leave an internal, leading, or trailing gap in the requested year. Obtain missing statements or review the source periods. A reviewed account-opening date may address only one continuous leading gap that ends immediately before the first source-supported period start. |
