...
- `coverage_hints.statement_period_years`, source-referenced `period_intervals`, and any `contextual_date_evidence` support the requested year. Each aggregated period interval has independently traceable `source_ref` and `end_source_ref` objects with its source file, page, and extracted line; the CSV shows both as `start=` and `end=`. A `direct-anchor` interval also records the source-bound period header and date anchor. A `medium` `inferred-chain` interval is only an exact-account, directly-adjacent derivation and remains review-required. A labelled prior-year opening balance, or an exact prior December 31 opening boundary ending in the requested year, is contextual evidence rather than a second statement period.
