...
4. Review the JSON and CSV before handing anything to downstream skills. For FBAR, pass a ready JSON directly; create `review-handoff` only after the user explicitly confirms every non-structural review gate, and never accept a structural `stop` gate.
