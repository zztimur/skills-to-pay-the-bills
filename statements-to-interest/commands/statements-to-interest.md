...
4. Review the preflight JSON/CSV. Pass it to `scripts/statements_to_interest.py extract` when it reports `ready-for-domain-extraction` with no review gates, or when it is a valid source-bound reviewed `one-institution` handoff with every required typed resolution. Use the same ordered PDF paths, because `--preflight-json` binds their normalized paths, byte sizes, SHA-256 digests, tax year, and institution. Never accept structural stops, genuine mixed years, or conflicting issuer evidence.
