...
4. Review the preflight JSON/CSV. Pass it to `scripts/statements_to_interest.py extract` only when it reports `ready-for-domain-extraction` with no review gates; use the same ordered PDF paths, because `--preflight-json` binds their normalized paths, byte sizes, SHA-256 digests, tax year, and institution.
