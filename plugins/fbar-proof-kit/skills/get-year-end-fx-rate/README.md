...
For manual sources, save the source page as PDF/HTML, take a screenshot, or download the source data when possible. The script copies that proof into the workpaper folder and hashes it. The `--proof-file` path has to be an existing regular file. A folder, broken link, or missing path is not proof, and the command stops before it makes a packet. `--source-note` is required. So is a real absolute `http://` or `https://` source URL. `--retrieved` has to be today or earlier, because a packet should not claim it retrieved a source from the future. Pass either `--proof-file` or a specific `--no-proof-file-reason` of at least 16 trimmed characters; the no-proof path records that reason and prints one caveat instead of pretending the generated workpaper PDF is source evidence.
...
The workpaper machinery — folder naming, the `workpaper.md` / `workpaper.json` / `workpaper.pdf` renderers, and the copied-and-hashed source-proof schema — lives in [`workpaper-kit`](https://github.com/zztimur/skills-to-pay-the-bills/tree/main/workpaper-kit) in the source repository, vendored here as `scripts/_workpaper.py` and shared with `get-yearly-fx-rate` so both proof packets look the same. The installed skill runs standalone: it imports this local copy, never `workpaper-kit` at runtime. For shared proof-packet changes, edit the canonical `workpaper-kit/workpaper.py`, never the generated copy; the pre-commit hook re-syncs it (or run `workpaper-kit/sync.sh`). Standalone installs do not include the canonical kit. The Treasury lookup, the `reject_average_language` rule, and currency handling stay here in the skill.
