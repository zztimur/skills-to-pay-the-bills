...
Repeat `--accept-gate` for every code in `review_gates`. The handoff records the original preflight path and SHA-256 plus the accepted gate codes. For ambiguous/unknown currency, account, year, opt-in issuer, or out-of-period generated-on metadata gates it also records source-bound structured reviewer resolutions. For an opt-in issuer gate, pass `--confirm-institution "Name"`; for an out-of-period generated-on date, pass `--confirm-generated-on-date YYYY-MM-DD` once for each exact source-labelled date. The latter remains document metadata and never becomes statement-period coverage. `extract-account` rejects a raw `review-required` JSON, an incomplete reviewed handoff, missing/mismatched required resolutions, a handoff with a structural gate, or a handoff whose source preflight changed after review.
