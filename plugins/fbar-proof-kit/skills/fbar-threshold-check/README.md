...
The extraction JSON carries explicit evidence, integrity, value-model, time-alignment, parser-coverage, and data-sufficiency cards. Its currency-agnostic geometry parser identifies Date/Description/Debit/Credit/Balance roles, supports source-resolved full dates and ISO timestamps, ignores description numbers, and selects only the final Balance/Saldo column. A debit/credit table or explicitly signed Amount/Valor/Monto column can reconstruct balances from a labelled opening only when every movement reconciles to a labelled close; an activity-free period requires equal opening and closing values. It remains diagnostic. Unsupported, unsigned, or omitted dated rows and failed reconciliation become a visible `parser-coverage-defect`, never a quiet partial success. An extraction with no usable observations is `unresolved-no-observations`, not formal evidence.
